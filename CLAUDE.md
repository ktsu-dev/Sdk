...
- **Sdk.Unity/**: Unity managed plug-in SDK
  ...
  - The packaged `.gitignore` negates two of its own generic rules for Unity, because both eat
    Unity *source*: `**/[Pp]ackages/*` (written for a NuGet restore folder; Unity's `Packages/`
    holds the manifest and lock file) and `*.meta` (written for the Visual Studio C++ artifact;
    Unity generates one `.meta` per asset carrying the GUID scenes and prefabs reference). Both
    negations are scoped to the asset tree so the artifacts they were written for stay ignored.
    Unity's generated caches (`Library/`, `Temp/`, `Logs/`) are deliberately *not* added: the
    names are only caches beside an `Assets/` folder, and `Library/` repo-wide would catch this
    repository's own `examples/demos/Library`. They belong in the Unity project's `.gitignore`.
  - A negation at the repository root does **not** reach `examples/`. `examples/.gitignore` is
    generated by `_KtsuSyncStyleConfigFiles` from whichever published ktsu.Sdk version
    `examples/global.json` pins, and for a given path the deeper file's matching rule wins - so a
    stale copy of `**/[Pp]ackages/*` there beats a negation at the root. The Unity demo carries
    its own `UnityProject/.gitignore` for that reason. A brand-new rule with no counterpart in the
    published copy, like Godot's `.godot/`, does reach `examples/` and needs no workaround. Never
    hand-edit `examples/.gitignore`: it is an output, and the next example build reverts it.
...
- **KTSU0008 (Error)**: Package reference overrides the shared framework. Flags a fully private `PackageReference` to a package the target framework's shared framework also supplies (`System.Text.Json`, `System.Memory`, and the rest of the targeting pack's `PackageOverrides.txt`) at a version above what that framework ships. Such a reference is not pruned, so it resolves a real compile assembly and the project binds against the higher assembly version; `PrivateAssets="all"` then keeps the dependency out of the produced package, so a consumer on that framework resolves nothing and fails with `FileNotFoundException` — binds roll forward but never backward. Invisible in the producing build and in its nuspec; it surfaces only when something consumes the packed artifact. This is a hazard **KTSU0006's own code fix creates**: it writes the reference at whatever version the analyzer observed, which for a framework-supplied package can be the higher one. See ktsu-dev/Sdk#44.
  - Three conditions must hold together, and each rules out a case that is not actually broken: the package is framework-supplied; it resolved a real compile assembly above the framework's version (at or below it NuGet prunes the reference, so there is nothing to bind against); and the reference is *fully private*, meaning the dependency is absent from the produced package. A reference that still flows carries the higher version to the consumer and resolves there, so it is not reported. "Fully private" is the same notion KTSU0007 enforces, and both now read it from the shared `PrivateAssets` helper. Two other helpers were extracted at the same time, for the same reason — SonarCloud's duplication gate on #50 caught that each rule had grown its own copy: `BuildFileLookup` now owns finding a build-generated input in `AdditionalFiles` by name (which KTSU0005, KTSU0006 and KTSU0008 each had a byte-identical copy of), finding the project file, matching a line that declares a package, and turning that line into a diagnostic `Location` (which KTSU0007 and KTSU0008 each spelled slightly differently). Adding a fourth package-graph rule should reach for these rather than copy them again.
  - The check is per target framework by construction, which is what makes "the lowest supported framework" fall out for free: the `net10.0` inner build of a `net10.0;net9.0` project sees the reference pruned and stays silent, while the `net9.0` one sees the override and reports. No cross-framework reasoning is needed in the analyzer.
  - A code fixer pins the version to what the framework ships — in `Directory.Packages.props` under central package management, otherwise on the `PackageReference` — and adds `NoWarn="NU1510"` to the item. The `NoWarn` is not incidental: once pinned to the floor the reference becomes prunable and NuGet reports NU1510 ("provided by the framework, remove it"), which is the opposite of what KTSU0006 demands, so the two rules can only be satisfied together with that scoped suppression.
  - The diagnostic is reported at the declaring line in `Directory.Packages.props` (preferred, since that is where a centrally managed version has to change) or else in the project file, not at a syntax-tree location — same generated-code drop that KTSU0001/KTSU0002/KTSU0007 were each fixed for.
...
**Package-graph analyzer inputs**: KTSU0005, KTSU0006 and KTSU0008 require solution-wide / post-restore facts that a per-project Roslyn analyzer cannot observe on its own. The SDK targets `_KtsuGenerateOrphanedPackageVersionInputs`, `_KtsuGenerateTransitivePackageInputs` and `_KtsuGenerateFrameworkOverrideInputs` (in `Sdk/Sdk.targets`) compute these facts at build time and surface them to the analyzers as `AdditionalFiles` (orphan list, assembly→package map, direct-package set, framework-supplied package list, per-TFM resolution facts), alongside the `Directory.Packages.props` that the KTSU0005/KTSU0006/KTSU0008 fixers edit via `AdditionalDocuments`. The project file is added to `AdditionalFiles` by a static `ItemGroup` rather than by any of the targets, so disabling one analyzer does not take another's code fix (or KTSU0007's) with it.

KTSU0008's two inputs come from MSBuild state that only exists after `ResolveReferences`: `@(PackageConflictOverrides)`, whose `OverriddenPackages` metadata *is* the targeting pack's `PackageOverrides.txt` (`packageId|version` entries), and `@(ResolvedCompileFileDefinitions)`, which carries a package's real compile assembly only when that package beat the framework. The overrides metadata keeps its embedded newlines through an MSBuild transform, so it is written out verbatim and tokenized on semicolons *and* newlines by the analyzer rather than reshaped in MSBuild — that is deliberate, and a reshaping "cleanup" there would silently produce one unparseable line.
...
- `EnableStrictModeForBaselineValidation=true` — governs how strict a comparison against a published
  baseline would be, if one runs. No `PackageValidationBaselineVersion` is set here or in any consumer,
  so baseline validation does not currently run.
