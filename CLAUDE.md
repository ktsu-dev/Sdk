...
These properties are passed to analyzers via `CompilerVisibleProperty`: `IsTestProject`, `TestProjectExists`, `TestProjectNamespace`, `TargetFramework`, `TargetFrameworkIdentifier`, `HasSourceLinkGitHub`, `HasSourceLinkAzureRepos`, `HasPolyfill`, `HasSystemMemory`, `HasSystemThreadingTasksExtensions`, `ManagePackageVersionsCentrally`.
